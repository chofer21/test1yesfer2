...
}else if($opcion == 'nuevo_tiempo'){
    
    $des = $_REQUEST['descripcion'];
    $val = $_REQUEST['valor'];
    
    qr("insert into configuracion (
                                    descripcion
                                   ,valor   
                                 ) VALUES (
                                    '".$des."'  -- descripcion - IN varchar(20)
                                   ,'".$val."'  -- valor - IN varchar(20)
                                 )");
    
    echo "<script>
                window.location.href ='../vistas/tiempo_entre_lectura.php';
        </script>";
    
}else if($opcion == 'config_tabla'){
    
        $fila = $_REQUEST['filas'];
        $col = $_REQUEST['col'];
        
        $existe = q1("select * from tamano_tabla");
        
        if($existe['filas']!=''){
            
            qr("update tamano_tabla set filas = ".$fila.", col = ".$col."");
            
        }else{
        
        qr("insert into tamano_tabla (
                                        filas
                                       ,col
                                     ) VALUES (
                                        ".$fila."
        ...
        echo "<script>
                window.location.href ='../vistas/nuevo_mapa.php';
        </script>";
