...
echo "<title>Mapa</title>";

$con_tabla = q("select id,descripcion,pos_x,pos_y,tipo from nodo where nivel = ".$ntabla."");
...
<div class="well">
    ...
    <input type='hidden' ntabla='<?php echo $ntabla ?>' id='ntabla'>
    ...
    <button id='asignar_salida' class="btn btn-warning">
        Asignar/Retirar Salida
    </button>
    
    <!--<button id='agregar_nivel' class="btn btn-primary">
        Agregar Nivel
    </button>-->
...
$tamano = q1("select * from tamano_tabla");
$filas = $tamano['filas'];
$col = $tamano['col'];

echo "<table border='1' id='table1' align='center'>";
for($ti=1;$ti<=$filas;$ti++){
    echo "<tr>";
        for($di=1;$di<=$col;$di++){
            $attrs = "";
            if($tab[$di][$ti]){
            ...
            echo "<td $attrs col='$di' row='$ti' ntabla='".$ntabla."' title='".$tab[$di][$ti]["id"]."'>"; 
            echo"</td>";
        }
    echo "</tr>";
}
...
$consulta_caminos = "SELECT id, inicio, fin 
	             FROM camino where nivel = ".$ntabla."";

$con_caminos = q($consulta_caminos);
$sensores_actuales = q("select * from camino_sensor where nivel = ".$ntabla."");
$leds_actuales = q("select * from camino_led where nivel = ".$ntabla."");
...
if(crear){
   ...
   if( $(this).css("background-color") === "<?php echo $color_nodo; ?>" || $(this).css("background-color") === "<?php echo $color_salida; ?>" ){
       ...
       crear_nodo("nodo" + cont_nodo, $t.attr("col"), $t.attr("row"), $t.attr("ntabla"));
...
if( $(this).css("background-color") === "<?php echo $color_nodo; ?>" ){  
	...
	if( array_caminos.indexOf(nodo_ini + "-" + nodo_fin) != '-1' || array_caminos.indexOf(nodo_fin + "-" + nodo_ini) != '-1' ){
	    ...
	    var camino_id = unir_nodos(nodo_ini,nodo_fin,distancia,tabla);
            
            cl(camino_id);
	    array_caminos.push(nodo_ini+"-"+nodo_fin);
	    pintar_camino(camino_id, ini_col,fin_col, ini_row,fin_row,tabla);
...
if(asignar){
    ...
    var ntabla = $(this).attr("ntabla");
    
    if(camino_id){   cargar_sensores(camino_id,ntabla);  }
...
function pintar_camino(camino_id, ini_col,fin_col, ini_row,fin_row,ntabla){
...
$("#agregar_nivel").click(function(){
   ...
   ntabla = $("#ntabla").attr("ntabla");
   ntabla = +ntabla+1;
   window.open("crear_mapa.php?ntabla="+ntabla);   
...
$("#asignar_salida").click(function(){    
    ...
    if(salida){
         salida = false;
         $(this).text("Asignar/Retirar Salida");
         $("#crear_nodos").show();
        $("#asignar_leds").show();
         $("#unir_puntos").show();
        $("#asignar_salida").show();
...
$(document).on("click",".b_asignar_sensor",function(){
    ...
    if(camino_id && sensor_id){
      asignar_sensores(camino_id,sensor_id,$this,ntabla);
    } 
...
$(document).on("click",".b_asignar_led",function(){
    ...
    asignar_led(camino_id,sensor_id,sentido,$this,ntabla);
...
function asignar_sensores(camino_id,sensor_id,$this,ntabla){
...
function asignar_led(camino_id,sensor_id,sentido,$this,ntabla){
...
function cargar_leds(camino_id,ntabla){    
            ...
            $.each(data.datos, function(index, tt) {
                ...
                if(tt["asignado"]==='no'){
                    table += "<tr>";
                        ...
                        table += "</td>";
                        table += "<td style='text-align:center'>";
                            table +="<button class='b_asignar_led btn btn-success' index='"+index+"' ntabla='"+ntabla+"' camino_id = '"+ camino_id +"' led_id='"+tt["id"]+"'>asignar</button>";                
                        table += "</td>";
                    table +="</tr>";
                    ...
                    table_asignados += "<tr>";
                        ...
                        table_asignados += "<td style='text-align:center'>";
                            table_asignados +="<button class='b_retirar_led btn btn-success'  ntabla='"+ntabla+"' camino_id = '"+ camino_id +"' sensor_id='"+tt["id"]+"'>retirar</button>";                
                        table_asignados += "</td>";
                    table_asignados +="</tr>";
                }
...
function cargar_sensores(camino_id,ntabla){    
            ...
            $.each(data, function(index, tt) {
                ...
                if(tt["asignado"]==='no'){
                    table += "<tr>";
                        ...
                        table += "</td>";
                        table += "<td style='text-align:center'>";
                            table +="<button class='b_asignar_sensor btn btn-success' ntabla='"+ntabla+"' camino_id = '"+ camino_id +"' sensor_id='"+tt["id"]+"'>asignar</button>";                
                        table += "</td>";
                    table +="</tr>";
                    ...
                    table_asignados += "<tr>";
                        ...
                        table_asignados += "</td>";
                        table_asignados += "<td style='text-align:center'>";
                            table_asignados +="<button class='b_retirar_sensor btn btn-success' ntabla='"+ntabla+"' camino_id = '"+ camino_id +"' sensor_id='"+tt["id"]+"'>retirar</button>";                
                        table_asignados += "</td>";
                    table_asignados +="</tr>";
                }
...
function crear_nodo(nodo,pos_x,pos_y,ntabla){
    cl(nodo);
...
function unir_nodos(nodo_ini,nodo_fin,distancia,ntabla){
